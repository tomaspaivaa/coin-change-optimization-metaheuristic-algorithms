...
float trepaColinas(int solution[], float *coinsTypeArray, struct info data, int numIter, int percentage) {
    ...
    cost = evaluateIndividual(solution,data, coinsTypeArray, &valid);
    for (i = 0; i < numIter; i++) {
        generateNeighbor(solution, newSol, data.numGenes, data.target, coinsTypeArray);

        costNeighbour =  evaluateIndividual(newSol,data, coinsTypeArray, &valid);
        int random_percentage = random_l_h(0, 100);
        // Aceita vizinho se o custo diminuir (problema de minimização)
        if ((valid && costNeighbour <= cost) || random_percentage < percentage) {
